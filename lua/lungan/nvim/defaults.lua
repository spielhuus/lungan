...
local function get_plugin_path(plugin_name)
  ...
  for _, path in ipairs(vim.split(rtp, ",")) do
    if vim.fn.fnamemodify(path, ":t") == plugin_name then
      return path
    end
  end
  return nil
...
M.default_prompts = true
-- prompt_path = {
--   vim.env.HOME .. "/.config/lungan",
--   "samples/prompts",
--   ".nvim/lungan",
-- },
M.prompt_path = function()
  local path = {
    vim.env.HOME .. "/.config/lungan",
    ".nvim/lungan",
  }
...
M.linewidth = 80 -- the linewidth for the textwrapper
M.repl_show = true
