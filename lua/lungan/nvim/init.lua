...
M.setup = function(opts)
	...
	vim.api.nvim_create_user_command("Lg", function(arg)
		...
		elseif arg.args == "Notebooks" then
			...
			for i, entry in ipairs(M.prompts()) do
				if entry.data:frontmatter().command and entry.data:frontmatter().command == arg.args then
					print("open chat: " .. arg.args)
					local chat = require("lungan.nvim.chat"):new(M.options, arg, entry)
					chat:open()
					table.insert(M.chats, chat)
					return
				end
		...
		nargs = "?",
		complete = function()
			local prompts = { "Attach", "Notebooks", "Chat", "Run", "Toggle", "Open" }
			for i, entry in ipairs(M.prompts()) do
				if entry.data:frontmatter().command then
					table.insert(prompts, entry.data:frontmatter().command)
				end
			end
