...
function Markdown:_parse(lines)
  ...
  while i <= #lines do
    ...
    if frontmatter and i == 1 then
      ...
      --TODO: self.fm = require("lungan.yaml"):new(nil, yaml)
      -- print("=========================================")
      -- print("IN: " .. table.concat(yaml, "\n"))
      local mes
      self.fm, mes = require("yalua.yalua").decode(table.concat(yaml, "\n") .. "\n")
      if self.fm == nil then
        print(mes)
      end
      -- print("load yaml: " .. vim.inspect(self.fm))
    elseif chat then
